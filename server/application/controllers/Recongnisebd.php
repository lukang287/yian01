...
class Recongnisebd extends CI_Controller{
    ...
    public function index(){
        ...
        $res = $client->asr(file_get_contents(dirname(__FILE__).'/16k.pcm'), 'pcm', 16000, array(
            'dev_pid' => 1536,'cuid'=> $yian_track_id, 'token' => $access_token
        ));

        //echo 'baidu res - '.var_export($res, true);

        api_return_json(API_RET_SUCCESS, '', json_decode($res, true));
        return;
    }
    ...
    private function get_baidu_accessToken(){
        ...
        $ret = $client->get('https://openapi.baidu.com/oauth/2.0/token', $req);
